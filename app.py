...
# Normalize image by resizing (128 x 128) and preprocessing
# Method == 1: Canny
# Method == 2: Grayscale
...
# PCA, to be used with eigen vector
...
# img_type: canny or bw (grayscale)
...
def predict_image(image, img_type, grades, k=1):
    ...
    training_raw_data = np.array(training_raw_data, dtype='f') # diubah dari list jadi array
    training_labels = np.array(training_labels, dtype='f') # diubah dari list jadi array

    prediction_raw_data = []

    pixels = img_to_feature_vector(image) #mengubah dari matriks gambar menjadi array #gambar yg mau di predict
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Receive uploaded file #simpan di server temporarily
            file.save(file_path)

            # Classify_image and return JSON
            grades = ['A', 'B', 'C'] #ngecek folder ada ga  folder A B C??
...
if __name__ == "__main__": #FLASK harus ada ini buat jalanin semua
    app.run()
